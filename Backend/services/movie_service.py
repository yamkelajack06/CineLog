...
class MovieService:
    ...
    def get_movie_details(movie_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"movie/{movie_id}?append_to_response=videos,images,credits")

    # user gets details of a specific tv show
    @staticmethod
    def get_tv_details(tv_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"tv/{tv_id}?append_to_response=videos,images,credits")

    # gets trending movies
    @staticmethod
    def get_trending_movies() -> ApiResponse:
        return Movie_Utils.make_request("trending/movie/day?language=en-US")

    # gets trending this week (movies + tv)
    @staticmethod
    def get_trending_this_week() -> ApiResponse:
        return Movie_Utils.make_request("trending/all/week?language=en-US")

    # get top rated movies
    @staticmethod
    def get_top_rated_movies() -> ApiResponse:
        return Movie_Utils.make_request("movie/top_rated?language=en-US&page=1")

    # gets movies by genre
    @staticmethod
    def get_movies_by_genre(genre_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"discover/movie?with_genres={genre_id}&language=en-US&page=1")

    # get upcoming releases
    @staticmethod
    def get_upcoming_movies() -> ApiResponse:
        return Movie_Utils.make_request("movie/upcoming?language=en-US&page=1")

    # gets popular movies
    @staticmethod
    def get_popular_movies() -> ApiResponse:
        return Movie_Utils.make_request("movie/popular?language=en-US&page=1")

    # gets similar movies to the searched/viewed movie
    @staticmethod
    def get_similar_movies(movie_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"movie/{movie_id}/similar?language=en-US&page=1")

    # gets personalized movie recommendations based on a movie
    @staticmethod
    def get_movie_recommendations(movie_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"movie/{movie_id}/recommendations?language=en-US&page=1")

    # gets cast and crew for a movie
    @staticmethod
    def get_movie_credits(movie_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"movie/{movie_id}/credits")

    # gets cast and crew for a tv show
    @staticmethod
    def get_tv_credits(tv_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"tv/{tv_id}/credits")

    # gets videos (trailers, teasers) for a movie
    @staticmethod
    def get_movie_videos(movie_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"movie/{movie_id}/videos?language=en-US")

    # gets videos (trailers, teasers) for a tv show
    @staticmethod
    def get_tv_videos(tv_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"tv/{tv_id}/videos?language=en-US")

    # get movie by actor/director person id
    @staticmethod
    def get_movies_by_person(person_id: int) -> ApiResponse:
        return Movie_Utils.make_request(f"person/{person_id}/movie_credits")

    # gets movies by release year
    @staticmethod
    def get_movies_by_year(year: int) -> ApiResponse:
        return Movie_Utils.make_request(f"discover/movie?primary_release_year={year}&language=en-US&page=1")
    ...
    def get_feed() -> dict:
        ...
        for key, endpoint in endpoints.items():
            response = Movie_Utils.make_request(endpoint)
